...
class Student:
    ...
    def __init__(self, name: str, courses: List[str], average_grade: float):
        """Student constructor."""
        self.name = name
    ...
    def __repr__(self):
        ...
        return f"{self.name} ({', '.join(self.courses)}), Average Grade: {self.average_grade}"

def filter_by_course(student_list: List[Student], course: str) -> List[Student]:
    """
...
def succeeding_students(student_list: List[Student]) -> List[Student]:
    """
    ...
    """
    return [student for student in student_list if not is_failing(student)]

def failing_students(student_list: List[Student]) -> List[Student]:
    """
    ...
    """
    return [student for student in student_list if is_failing(student)]

def sort_by_best_grade(student_list: List[Student]) -> List[Student]:
    """
    Return a sorted list of students by their average grade in descending order.

    Highest average grade students first.
    If a student is failing school (average grade less than 1.0) then don't return them in the list.
    ...
    """
    return sorted(succeeding_students(student_list), key=lambda student: (-student.average_grade, student.name))

def sort_by_worst_grade(student_list: List[Student]) -> List[Student]:
    """
