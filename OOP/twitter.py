...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    """
    fastest_growing_tweet = max(tweets, key=lambda tweet: tweet.retweets / tweet.time)
...
def sort_by_popularity(tweets: list) -> list:
    ...
    """
    sorted_tweets = sorted(tweets, key=lambda tweet: (tweet.retweets, -tweet.time), reverse=True)
...
def filter_by_hashtag(tweets: list, hashtag: str) -> list:
    ...
    """
    filtered_tweets = [tweet for tweet in tweets if hashtag.lower() in tweet.content.lower()]
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    >Tweet1 has 21 retweets and has the common hashtag.
    >Tweet2 has 19 retweets and has the common hashtag.
    ...
    for tweet in tweets:
        ...
        for hashtag in hashtags:
            hashtag_popularity[hashtag] = hashtag_popularity.get(hashtag, 0) + tweet.retweets

    sorted_hashtags = sorted(hashtag_popularity.items(), key=lambda x: (-x[1], x[0]))
    return [f"#{hashtag}" for hashtag, popularity in sorted_hashtags]
